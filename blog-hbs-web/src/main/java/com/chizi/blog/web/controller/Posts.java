...
public class Posts extends ViewBase {
    ...
    public String list(@RequestParam(required = false) Long author, @RequestParam(defaultValue = "0") Integer pageNo,
            @RequestParam(defaultValue = "20") Integer pageSize, Model model) {

        try {
            Paging<Post> respPaging = postService.paging(
                    new PostPagingCriteria().setAuthorId(author).setPageNo(pageNo).setPageSize(pageSize).toMap());
    ...
    public String findById(@PathVariable("postId") String postId, Model model) {
        ...
        try {
            ...
            postResp.setComments(commentService
                    .paging(new CommentPagingCriteria().setPostId(postId).setPageNo(0).setPageSize(100).toMap()));
